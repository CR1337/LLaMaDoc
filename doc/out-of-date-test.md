...
We use lower case latin and greek letters for scalars (e.g. $n, \gamma$), bold latin letters for vectors (e.g. $\bm{t}, \bm{p}$), calligraphic capital letters for functions (e.g. $\mathcal{T}, \mathcal{D}$) and fraktur letters for strings (e.g. $\mathfrak{c}, \mathfrak{d}$). We also use the symbol $\circ$ for the concatenation of strings and vectors.

To denote the $i$-th element of a vector $\bm{v}$ we use the notation $\bm{v}[i]$ and to denote the vector conataining the $i$-th to $j$-th (exclusive) elements of a vector $\bm{v}$ we use the notation $\bm{v}[i:j]$.
...
We first map the code $\mathfrak{c}$ and the user provided docstring $\mathfrak{d_u}$ to token ids $\bm{t_c} \in \mathbb{N}^o$ and $\bm{t_{d_u}} \in \mathbb{N}^n$ respectively:

$\bm{t_c} = \mathcal{T}(\mathfrak{c})$

$\bm{t_{d_u}} = \mathcal{T}(\mathfrak{d_u})$
...
$\bm{t_c} = (0, 1, 2, 3, 4, 5, 6, 3, 7, 8)$

$\bm{t_{d_u}} = (9, 10, 11, 3)$

### 2. Prediction

Next we construct the prompt $\mathfrak{p}=$ "\<code>" $\circ \mathfrak~{c}~\circ$ "\<docstring>" and its token vector $\bm{t_p} = \mathcal{T}(\mathfrak{p})$.
...
We let $\bm{t_{d_m}}$ be the token vector for the docstring generated by the model $M_G$ for the code prompt $\mathfrak{p}$:

$\bm{t_{d_m}} = \mathcal{G}_{M_G}(\bm{t_p})$
...
$\bm{t_{d_m}} = (12, 10, 11, 3)$


### 3. Logit and probability computation

Let $\mathcal{L}_{M_G}:\mathbb{N}^*\to\mathbb{R}^*$ be the function that computes the [logits](https://en.wikipedia.org/wiki/Logit) for the next token generated by the model $M_G$ for given prompt tokens $\bm{t}\in\mathbb{N}^*$. By using the [$\text{softmax}$](https://en.wikipedia.org/wiki/Softmax_function) function we obtain the probability distribution $\bm{f}(\bm{t})$ for the next token over the vocabulary:

$\bm{f}(\bm{t}) = \text{softmax}(\mathcal{L}_{M_G}(\bm{t}))$

We construct a series of prompt token vectors $\bm{t_i^u}$ by concatenating the code and the first $i$ tokens of the user provided docstring:

$\forall i \in \{0,1,\ldots,n\}: \bm{t_i^u} = \bm{t_p} \circ \bm{t_{d_u}}[0:i]$

and a series of prompt token vectors $\bm{t_i^m}$ by concatenating the code and the first $i$ tokens of the model generated docstring:

$\forall i \in \{0,1,\ldots,m\}: \bm{t_i^m} = \bm{t_p} \circ \bm{t_{d_m}}[0:i]$

We compute the probability distributions $\bm{f_i^u}$ and $\bm{f_i^m}$ for the next token for each prompt $\bm{t_i^u}$ and $\bm{t_i^m}$:

$\forall i \in \{0,1,\ldots,n\}: \bm{f_i^u} = \text{softmax}(L_{M_G}(\bm{t_i^u}))$

$\forall i \in \{0,1,\ldots,m\}: \bm{f_i^m} = \text{softmax}(L_{M_G}(\bm{t_i^m}))$

Next we we obtain the probabilities $p_i^u$ and $p_i^m$ for each token $\bm{t_{d_u}}[i+1]$ and $\bm{t_{d_m}}[i+1]$ by looking them up in the probability distributions $\bm{f_i^u}$ and $\bm{f_i^m}$:

$\forall i \in \{0,1,\ldots,n-1\}: p_i^u = \bm{f_i^u}[\bm{t_{d_u}}[i+1]]$

$\forall i \in \{0,1,\ldots,m-1\}: p_i^m = \bm{f_i^m}[\bm{t_{d_m}}[i+1]]$

Resulting in the probability vectors $\bm{p_u}$ and $\bm{p_m}$:

$\bm{p_u} = (p_0^u, p_1^u, \ldots, p_{n-1}^u)$

$\bm{p_m} = (p_0^m, p_1^m, \ldots, p_{m-1}^m)$
...
And the prompts $\bm{t_i^m}$ would be:
```
<code>def double(x): return x * 2<docstring>
...
$\bm{p_u} = (0.1, 0.2, 0.5, 0.6)$

$\bm{p_m} = (0.9, 0.2, 0.5, 0.6)$
...
$\displaystyle{\mathcal{M}(\bm{p}) = \exp\left(\sum_{i=0}^{n-1} \mathcal{W}(i, n)\cdot ln~p_i\right)}$
...
Lastly we compute the measures $\mathcal{M}(\bm{p_u})$ and $\mathcal{M}(\bm{p_m})$ for the user provided docstring and the model generated docstring:

$\displaystyle{\mathcal{M}(\bm{p_u}) = \exp\left(0.67\cdot\ln 0.1 + 0.16\cdot\ln 0.2 + 0.12\cdot\ln 0.5 + 0.05\cdot\ln 0.6\right) = 0.14}$

$\displaystyle{\mathcal{M}(\bm{p_m}) = \exp\left(0.67\cdot\ln 0.9 + 0.16\cdot\ln 0.2 + 0.12\cdot\ln 0.5 + 0.05\cdot\ln 0.6\right) = 0.65}$

### 5. Evaluation

We can now define the evaluation function $\mathcal{E_P}: \Sigma^* \times \Sigma^*\to\{\text{true},\text{false}\}$ as follows:

$\displaystyle{\mathcal{E_P}(\mathfrak{c}, \mathfrak{d_u}) = \frac{\mathcal{M}(\bm{p_u})}{\mathcal{M}(\bm{p_m})} \leq \theta}$
...
$\bm{e_c} = \mathcal{P}_{M_E}(\mathfrak{c})$

$\bm{e_{d_u}} = \mathcal{P}_{M_E}(\mathfrak{d_u})$

$\bm{e_{d_m}} = \mathcal{P}_{M_E}(\mathfrak{d_m})$
...
$\bm{e_{d_u}} = (0.2, 0.8, 0.4)$

$\bm{e_{d_m}} = (0.1, 0.3, 0.5)$
...
$d_{\mathfrak{d_u}} = \mathcal{D}(\bm{e_c}, \bm{e_{d_u}})$

$d_{\mathfrak{d_m}} = \mathcal{D}(\bm{e_c}, \bm{e_{d_m}})$
